...
class ProjectProjectExt(models.Model):
   ...
   # Verificar si todos los aprobadores ya aprobaron
    ...
    def _compute_helpdesk_ticket_count(self):
        # Si Helpdesk no está instalado, poner 0 por defecto sin error
        if not self.env.registry.get('helpdesk.ticket'):
            for project in self:
                project.helpdesk_ticket_count = 0
            return

        Ticket = self.env['helpdesk.ticket'].sudo()
        # Agregar eficientemente los conteos por proyecto
        data = Ticket.read_group(
            [('project_id', 'in', self.ids)],
            ['project_id'],
            ['project_id']
        )
        count_map = {d['project_id'][0]: d['project_id_count'] for d in data}
        for project in self:
            project.helpdesk_ticket_count = count_map.get(project.id, 0)

    def action_view_tickets(self):
        self.ensure_one()
        if not self.env.registry.get('helpdesk.ticket'):
            raise UserError("El módulo Helpdesk no está instalado en esta base de datos.")
        return {
            'name': 'Tickets',
            'type': 'ir.actions.act_window',
