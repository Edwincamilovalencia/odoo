...
class RetainCallHistory(models.Model):
    ...
    def action_sincronizar_historial(self):
        try:
            ...
            nuevas, actualizadas, transcripciones_encontradas, descripciones_encontradas = self._sync_basic_call_data(total_llamadas)
            ...
            if nuevas < 20:  # Si hay pocas llamadas nuevas, hacer completado
                transcripciones_adicionales, agentes_adicionales, descripciones_adicionales = self._complete_missing_data()
                agentes_exhaustivos = self._exhaustive_agent_search()
            ...
            traducidas = 0
            for llamada in llamadas_ingles:
                desc_es = self._translate_to_spanish(llamada.description_llamada)
                if desc_es != llamada.description_llamada:
                    llamada.description_llamada = desc_es
                    traducidas += 1
    ...
    def _complete_missing_data(self):
        ...
        # Procesar en lotes más grandes para ser más rápido
        batch_size = 25
        for i in range(0, len(llamadas_con_call_id), batch_size):
            ...
            for llamada in batch:
                try:
                    ...
                    transcript_response = requests.get(transcript_url, headers=headers, timeout=5)  # Timeout más corto
                    ...
                    if transcript_response.status_code == 200:
                        ...
                        # Solo buscar transcripción si falta
                        ...
                        # Solo buscar descripción si falta
