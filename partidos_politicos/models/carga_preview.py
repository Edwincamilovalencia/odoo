...
class PartidoCargaPreview(models.Model):
    ...
    def _detect_kind(self, content):
        # Preferred by explicit type/extension
        ...
        if ext == 'xls' and content[:8] == b"\xD0\xCF\x11\xE0\xA1\xB1\x1A\xE1":
            return 'xls'
        # If magic mismatches, still return ext; caller can fallback to csv
        return ext
    ...
    def _render_table(self, rows, header=None):
        ...
        # Estilos: ocupar todo el ancho disponible y evitar recortes
        table_css = (
